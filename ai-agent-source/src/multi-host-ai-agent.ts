...
import AdvancedSmartContractInterface from "./advanced-smart-contract-fixed";
import AdvancedHostAvailabilityManager from "./advanced-host-availability";
import PersistentSessionManager from "./persistent-session-manager";
import MultiHostManager, { HostProfile, HostSearchFilters } from "./multi-host-manager";

class MultiHostAIAgent {
  private xmtpClient: Client | null = null;
  private wallet: Wallet;
  private smartContract: AdvancedSmartContractInterface;
  private availabilityManager: AdvancedHostAvailabilityManager;
  private sessionManager: PersistentSessionManager;
  private multiHostManager: MultiHostManager;
  private isRunning: boolean = false;
  private currentHostAddress: string;
  ...
  constructor() {
    ...
    this.smartContract = new AdvancedSmartContractInterface();
    this.smartContract.initializeWallet(privateKey);
    
    this.sessionManager = new PersistentSessionManager();
    this.multiHostManager = new MultiHostManager(this.smartContract, this.sessionManager);
    this.availabilityManager = new AdvancedHostAvailabilityManager(this.smartContract, this.currentHostAddress);
  ...
  private async ensureHostRegistration(): Promise<void> {
    try {
      ...
      if (!hostInfo.isRegistered) {
        ...
        await this.smartContract.registerAsAdvancedHost(
          JSON.stringify({
...
What would you like to do today?`;
  ...
  private async handleHostSearch(message: string, session: any): Promise<string> {
    try {
  ...
  private async handleHostComparison(message: string, session: any): Promise<string> {
    try {
  ...
  private async handleAvailabilityCheck(message: string, session: any): Promise<string> {
    try {
  ...
  private async handleMeetingBooking(message: string, session: any): Promise<string> {
    try {
      ...
      const availableSlots = await this.smartContract.getAvailableSlots(
      ...
      try {
        const result = await this.smartContract.scheduleMeetingAdvanced(
          bookingDetails.hostAddress,
          startTime,
          endTime,
          price,
          bookingDetails.templateId || '0',
...
Transaction Hash: ${result.txHash}`;
  ...
  private async handleRecommendations(message: string, session: any): Promise<string> {
    try {
      // Parse preferences from message
      const preferences = this.parsePreferences(message);
      
      const recommendations = await this.multiHostManager.getRecommendedHosts(
        session.requesterAddress,
        preferences,
        5
      );
      ...
      recommendations.forEach((host, index) => {
        ...
        if (host.metadata.verified) response += `   ✅ Verified Host\n`;
        if (host.metadata.instantBooking) response += `   ⚡ Instant Booking\n`;
        response += `\n`;
      });
  ...
  private async handleSpecialtyBrowsing(message: string, session: any): Promise<string> {
    try {
  ...
  private async handleMarketplaceInfo(session: any): Promise<string> {
    try {
...
Just tell me what you need in natural language!`;
  ...
  private async handleGeneralQuery(message: string, session: any): Promise<string> {
  ...
  private parseSearchFilters(message: string): HostSearchFilters {
    const filters: HostSearchFilters = {};
    const lowerMessage = message.toLowerCase();
  ...
  private parseSortCriteria(message: string): 'rating' | 'price' | 'availability' | 'responseTime' {
    const lowerMessage = message.toLowerCase();
  ...
  private extractHostAddresses(message: string): string[] {
    // Extract Ethereum addresses from message
    const addressRegex = /0x[a-fA-F0-9]{40}/g;
    return message.match(addressRegex) || [];
  }

  private extractHostFromMessage(message: string): string | null {
  ...
  private parsePreferences(message: string): any {
    ...
    if (lowerMessage.includes('consulting')) preferences.specialty = 'Consulting';
    if (lowerMessage.includes('coaching')) preferences.specialty = 'Coaching';
    if (lowerMessage.includes('technical')) preferences.specialty = 'Technical Support';
    
    return preferences;
  }

  private formatHostSummary(host: HostProfile, index: number): string {
    ...
    if (host.metadata.verified) summary += ` | ✅ Verified`;
    if (host.metadata.instantBooking) summary += ` | ⚡ Instant`;
...
export default MultiHostAIAgent;
