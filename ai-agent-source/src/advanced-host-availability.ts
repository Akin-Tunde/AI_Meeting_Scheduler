...
interface AdvancedAvailabilitySlot {
  slotId: string;
  startTime: Date;
  endTime: Date;
  price: string;
...
class AdvancedHostAvailabilityManager {
  ...
  private availabilityCache: Map<string, AdvancedAvailabilitySlot> = new Map();
  private templatesCache: Map<string, MeetingTemplate> = new Map();
  private lastCacheUpdate: number = 0;
  private cacheValidityPeriod: number = 5 * 60 * 1000; // 5 minutes

  constructor(smartContract: AdvancedSmartContractInterface, hostAddress: string) {
  ...
  async addAvailabilitySlot(
    ...
    try {
      const result = await this.smartContract.addAvailabilitySlot(
        startTime,
        endTime,
        price,
        isRecurring,
      ...
      // Update cache
      const slot: AdvancedAvailabilitySlot = {
  ...
  async getAvailabilityForDate(dateQuery?: string): Promise<string[]> {
    try {
  ...
  async getAvailableSlots(fromTime: Date, toTime: Date): Promise<AdvancedAvailabilitySlot[]> {
    try {
      ...
      const availableSlots: AdvancedAvailabilitySlot[] = [];
  ...
  async createMeetingTemplate(
    ...
    price: string,
    pricingModel: number = 0 // Fixed pricing by default
  ): Promise<string> {
    try {
      const result = await this.smartContract.createMeetingTemplate(
        title,
        description,
        durationMinutes,
        price,
  ...
  async getHostTemplates(): Promise<MeetingTemplate[]> {
    try {
  ...
  async addRecurringAvailability(
    ...
    endTime: Date,
    price: string,
    recurringDays: number[], // Days of week (0-6, 0=Sunday)
    weeksCount: number,
    metadata: string = ''
  ): Promise<string[]> {
      ...
      for (let week = 0; week < weeksCount; week++) {
        for (const dayOfWeek of recurringDays) {
          ...
          if (slotStart > new Date()) {
            const slotId = await this.addAvailabilitySlot(
              slotStart,
              slotEnd,
              price,
              false, // Individual slots, not recurring
              0,
              `${metadata} - Week ${week + 1}, ${this.getDayName(dayOfWeek)}`
            );
            slotIds.push(slotId);
          }
        }
  ...
  async addBusinessHoursAvailability(
    startDate: Date,
    endDate: Date,
          ...
          for (let hour = dailyStartHour; hour < dailyEndHour; hour += slotDurationMinutes / 60) {
            ...
            if (slotStart > new Date()) {
              const slotId = await this.addAvailabilitySlot(
                ...
                false,
                0,
                `Business hours - ${this.formatTimeSlot(slotStart, slotEnd)}`
              );
              slotIds.push(slotId);
            }
          }
  ...
  async getAvailabilityAnalytics(fromDate: Date, toDate: Date): Promise<{
    totalSlots: number;
    bookedSlots: number;
    availableSlots: number;
    totalPotentialEarnings: string;
    averageSlotPrice: string;
  ...
  private async refreshAvailabilityCache(fromTime: Date, toTime: Date): Promise<void> {
    try {
      const slots = await this.smartContract.getAvailableSlots(this.hostAddress, fromTime, toTime);
      
      this.availabilityCache.clear();
      for (const slot of slots) {
        const advancedSlot: AdvancedAvailabilitySlot = {
          slotId: slot.slotId,
  ...
  private parseDateQuery(dateQuery?: string): { fromTime: Date; toTime: Date } {
    const now = new Date();
    let fromTime: Date;
    let toTime: Date;
  ...
  private getNextWeekday(date: Date, targetDay: number): Date {
  ...
  private formatSlotForDisplay(slot: AdvancedAvailabilitySlot): string {
  ...
  private formatTimeSlot(startTime: Date, endTime: Date): string {
  ...
  async generateDefaultSchedule(
    ...
    dailyStartHour: number = 9,
    dailyEndHour: number = 17,
    slotDurationMinutes: number = 60,
    price: string = '0.01'
  ): Promise<string[]> {
    ...
    return await this.addBusinessHoursAvailability(
      startDate,
      endDate,
      dailyStartHour,
      dailyEndHour,
...
export default AdvancedHostAvailabilityManager;
