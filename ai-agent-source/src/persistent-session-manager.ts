import SupabaseClient, { Session, ConversationMessage, SessionState } from './supabase-client';
...
class PersistentSessionManager {
  ...
  async updateSession(sessionId: string, updates: Partial<SessionData>): Promise<SessionData> {
    ...
    try {
      ...
      if (updates.meetingStartTime) dbUpdates.meeting_start_time = updates.meetingStartTime.toISOString();
      if (updates.meetingEndTime) dbUpdates.meeting_end_time = updates.meetingEndTime.toISOString();
      if (updates.meetingDuration !== undefined) dbUpdates.meeting_duration = updates.meetingDuration;
      if (updates.meetingPrice !== undefined) dbUpdates.meeting_price = updates.meetingPrice;
      if (updates.meetingCurrency) dbUpdates.meeting_currency = updates.meetingCurrency;
      if (updates.meetingId) dbUpdates.meeting_id = updates.meetingId;
  ...
  async getSessionsByHost(hostAddress: string, status?: string): Promise<SessionData[]> {
    this.ensureInitialized();
  ...
  async getSessionsByRequester(requesterAddress: string, status?: string): Promise<SessionData[]> {
    this.ensureInitialized();
  ...
  // Message Management
  async addMessage(sessionId: string, messageType: 'user' | 'agent' | 'system', content: string, senderAddress?: string, xmtpMessageId?: string): Promise<void> {
    this.ensureInitialized();
  ...
  async updateSessionState(sessionId: string, step: string, stepData: Record<string, any> = {}): Promise<void> {
    this.ensureInitialized();
  ...
  async getCurrentSessionState(sessionId: string): Promise<{ step: string; data: Record<string, any> } | null> {
    this.ensureInitialized();
  ...
  // Utility Methods
  private convertFromDbSession(dbSession: Session): SessionData {
    return {
  ...
  private async startCleanupRoutine(): Promise<void> {
  ...
  subscribeToSessionUpdates(sessionId: string, callback: (session: SessionData) => void) {
    this.ensureInitialized();
  ...
  subscribeToNewMessages(sessionId: string, callback: (message: {
    type: 'user' | 'agent' | 'system';
    content: string;
    timestamp: Date;
    senderAddress?: string;
