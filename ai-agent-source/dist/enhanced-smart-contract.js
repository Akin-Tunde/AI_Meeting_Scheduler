...
class EnhancedSmartContractInterface {
    constructor() {
        ...
        this.contract = new ethers_1.ethers.Contract(CONTRACT_ADDRESS, ENHANCED_CONTRACT_ABI, this.provider);
    }
    // Initialize with a wallet for write operations
    initializeWallet(privateKey) {
        this.wallet = new ethers_1.ethers.Wallet(privateKey, this.provider);
        this.contract = new ethers_1.ethers.Contract(CONTRACT_ADDRESS, ENHANCED_CONTRACT_ABI, this.wallet);
    }
    // Host Registration Functions
    async registerAsHost(defaultPriceInEth, metadata) {
        if (!this.wallet) {
            throw new Error('Wallet not initialized');
        }
        try {
            const priceInWei = ethers_1.ethers.parseEther(defaultPriceInEth);
    ...
    async updateHostMetadata(metadata) {
        if (!this.wallet) {
            throw new Error('Wallet not initialized');
        }
        try {
    ...
    async isHostRegistered(hostAddress) {
        try {
    ...
    async getHostInfo(hostAddress) {
        try {
            ...
            return {
                ...
                defaultPrice: ethers_1.ethers.formatEther(result.defaultPrice),
    ...
    async updateAvailabilitySlot(startTime, isAvailable) {
        if (!this.wallet) {
            throw new Error('Wallet not initialized');
        }
        try {
    ...
    async getHostAvailabilityCount(hostAddress) {
        try {
    ...
    async getHostAvailabilitySlot(hostAddress, index) {
        try {
            ...
            return {
                ...
                price: ethers_1.ethers.formatEther(result.price)
            };
        }
        catch (error) {
    ...
    async getAvailableSlots(hostAddress, fromTime, toTime) {
        try {
            ...
            for (let i = 0; i < result.startTimes.length; i++) {
                slots.push({
                    ...
                    price: ethers_1.ethers.formatEther(result.prices[i])
                });
            }
            return slots;
        }
    ...
    async isSlotAvailable(hostAddress, startTime) {
        try {
...
exports.default = EnhancedSmartContractInterface;
