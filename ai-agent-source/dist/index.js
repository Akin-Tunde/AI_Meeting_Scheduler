...
const xmtp_client_1 = require("./xmtp-client");
const nlp_processor_1 = require("./nlp-processor");
const host_availability_1 = require("./host-availability");
...
const smartContract = new smart_contract_1.default();
async function main() {
    ...
    const xmtp = await (0, xmtp_client_1.initializeXMTP)(wallet);
    console.log(`XMTP client initialized for address: ${xmtp.address}`);
    console.log('Listening for messages...');
    await (0, xmtp_client_1.listenForMessages)(async (message, senderAddress) => {
        console.log(`Received message from ${senderAddress}: ${message}`);
        // Process message with NLP
        const nlpResponse = await (0, nlp_processor_1.processNLP)(message);
        let responseText = nlpResponse.answer || "I'm not sure how to respond to that. Can you rephrase?";
        // Extract entities
        const dateEntity = nlpResponse.entities.find((entity) => entity.entity === 'date');
        ...
        switch (nlpResponse.intent) {
            case 'check.availability':
                const availableSlots = (0, host_availability_1.getMockAvailability)(availabilityDate);
            ...
            case 'book.meeting':
                try {
                    ...
                    const session = (0, session_manager_1.createMeetingSession)(senderAddress, HOST_ADDRESS, availabilityDate || 'TBD', 30, // default 30 minutes
                    0.01, // price in ETH
                    'ETH');
            ...
            case 'cancel.meeting':
                ...
                const userSessions = (0, session_manager_1.getUserSessions)(senderAddress);
                if (userSessions.length > 0) {
                    ...
                    (0, session_manager_1.updateMeetingSessionStatus)(latestSession.id, 'cancelled');
                }
                else {
        ...
        await (0, xmtp_client_1.sendMessage)(senderAddress, responseText);
    });
}
